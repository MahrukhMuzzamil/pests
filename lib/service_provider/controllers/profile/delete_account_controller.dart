  ...
  Future<void> _deleteUserFromLeads(String userId) async {
    ...
    if (leadsSnapshot.docs.isNotEmpty) {
      for (var leadDoc in leadsSnapshot.docs) {
        final buyers = leadDoc.data()['buyers'] as List?;
        if (buyers != null) {
          final buyerToRemove = buyers.firstWhere(
                  (buyer) => buyer['userId'] == userId,
              orElse: () => null
          );
          if (buyerToRemove != null) {
            try {
              await firestore.collection('leads').doc(leadDoc.id).update({
                'buyers': FieldValue.arrayRemove([buyerToRemove])
              });
              print('Removed user from lead: ${leadDoc.id}');
            } catch (e) {
      ...
      print('No leads found for user: $userId');
    }
  }
